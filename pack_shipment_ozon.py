...
def get_orders_to_shipment(clientID, token, status):
    ...
    date_ship_from = (datetime.datetime.now() - datetime.timedelta(days=30)).strftime("%Y-%m-%d")
    ...
    print("get_orders_to_shipment: ", r)
    load = r.json()
    # print(load)
    posting_numbers = []
    delivery_method = []
    ...
    print("Posting number/ delivery method : ", posting_numbers, delivery_method)
...
def set_awaiting_deliver_for_order(order, clientID, token):
    ...
    headers = {"Client-Id": clientID, "Api-Key": token, "Content-Type": "application/json"}
    r = requests.post(url, headers=headers, json=json)
    print("set_awaiting_deliver_for_order 1: ", r)
    load = r.json()
    print(load)
...
def get_pakage_barcode(taskID, shop, clientID, token):
    url = "https://api-seller.ozon.ru/v2/posting/fbs/act/get-barcode"
    headers = {"Client-Id": clientID, "Api-Key": token, "Content-Type": "application/json"}
    json = {"id": taskID}
    r = requests.post(url, headers=headers, json=json)
    print("get_package_barcode: ", r)
    print(r.text)
    print(r)
    print(r.content)
    load = r.content
    with open(act_name := f'{datetime.datetime.now().strftime("%Y.%m.%d")} OZON {shop} barcode.png', 'wb') as f:
        f.write(load)
    return act_name

#203366890
# delivery_id = "1020001819824000"
# delivery_id = "203366890"
# shop, clientID, token = tokens[0]
#
# print(get_pakage_barcode(delivery_id, shop, clientID, token))
